# Development Environment Configuration for Transaction Validator
...
terraform {
  ...
  backend "s3" {
    bucket         = "transaction-validator-terraform-state-dev"
    key            = "dev/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
    dynamodb_table = "transaction-validator-terraform-locks-dev"
  }
}
